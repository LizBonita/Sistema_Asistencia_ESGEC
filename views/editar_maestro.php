...
$nombreMaestro = htmlspecialchars($maestro->nombre_completo_usuario ?? '');
$nombreUsuario = htmlspecialchars($maestro->nombre_usuario ?? '');
$rolActual = htmlspecialchars($maestro->rol_nombre_usuario ?? '');
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Editar Maestro - Sistema de Asistencia</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}body{font-family:'Roboto',sans-serif;background:radial-gradient(900px 420px at 0% 0%,rgba(39,132,211,.12),transparent 55%),radial-gradient(900px 420px at 100% 0%,rgba(34,197,94,.10),transparent 55%),var(--bg);color:var(--text);min-height:100vh}a{text-decoration:none;color:inherit}button,input,select{font-family:inherit}
        ...
        .header-actions{display:flex;gap:12px;flex-wrap:wrap}.header-btn{display:inline-flex;align-items:center;gap:10px;min-height:48px;padding:12px 20px;border-radius:999px;font-size:.95rem;font-weight:800;color:var(--white);background:rgba(255,255,255,.14);border:1px solid rgba(255,255,255,.16);box-shadow:var(--shadow-sm);backdrop-filter:blur(10px);transition:.18s ease;white-space:nowrap;cursor:pointer}.header-btn:hover{transform:translateY(-2px);background:rgba(255,255,255,.20)}.theme-toggle{cursor:pointer}
        .page-wrapper{width:min(700px,calc(100% - 32px));margin:28px auto 40px}
        ...
        .info-card{display:flex;align-items:center;gap:14px;padding:16px 18px;border-radius:16px;background:linear-gradient(135deg,rgba(27,57,106,.06),rgba(34,197,94,.04));border:1px solid var(--border);margin-bottom:22px}body.dark-mode .info-card{background:rgba(255,255,255,.04)}
        .info-avatar{width:52px;height:52px;border-radius:16px;background:linear-gradient(135deg,rgba(39,132,211,.16),rgba(34,197,94,.16));display:grid;place-items:center;color:var(--primary);font-weight:900;font-size:1.15rem;flex-shrink:0}body.dark-mode .info-avatar{color:#dce8ff}
        .info-data strong{display:block;font-size:1.05rem;font-weight:800}.info-data span{display:block;color:var(--text-muted);font-size:.85rem;margin-top:2px}
    ...
    <div class="top-header-inner">
        <div class="header-title">
            ...
            <div>
                ...
                <p>Modifica el tipo de contrato del maestro</p>
            </div>
        </div>
        <div class="header-actions">
            <a href="gestion_maestros.php" class="header-btn"><i class="fas fa-arrow-left"></i> Volver</a>
    ...
    <div class="form-card">
        <div class="form-card-head">
            ...
            <div>
                ...
                <p>Revisa la información y actualiza el contrato</p>
            </div>
        </div>
        <div class="form-body">
            <!-- Info de solo lectura -->
            <div class="info-card">
                <div class="info-avatar"><?php echo mb_strtoupper(mb_substr($nombreMaestro, 0, 1, 'UTF-8'), 'UTF-8') . (strpos($nombreMaestro, ' ') ? mb_strtoupper(mb_substr(explode(' ', $nombreMaestro)[1], 0, 1, 'UTF-8'), 'UTF-8') : ''); ?></div>
                <div class="info-data">
                    <strong><?php echo $nombreMaestro; ?></strong>
                    <span><i class="fas fa-user"></i> <?php echo $nombreUsuario; ?> &nbsp;·&nbsp; <i class="fas fa-shield-halved"></i> <?php echo $rolActual; ?></span>
                </div>
            </div>
            ...
            <form action="../controllers/ActualizarMaestroController.php" method="POST">
                ...
                <div class="form-group">
                    <label><i class="fas fa-file-contract"></i> Tipo de Contrato</label>
                ...
                <div class="form-actions">
                    ...
                    <a href="gestion_maestros.php" class="btn-cancel"><i class="fas fa-times"></i> Cancelar</a>
                </div>
            </form>
        </div>
    </div>
